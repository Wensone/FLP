...
splitl(_,_,[],[],[],[]).
goalo:-writeln('Vvedite spisok i razdeliteli:'), read(S), read(R1), read(R2), splitl(R1, R2, S, S1, S2, S3),
 writeln(S), writeln(S1), writeln(S2), writeln(S3).
...
goalt :-writeln('Vvedite spisok:'), read(X), min_elt(X,M), num_elt(X,M,1,R), writeln(R).

goalh :- writeln('Vash spisok:'), read(S), maxh(S,COUNT), print(S, COUNT, R), writeln(R).
